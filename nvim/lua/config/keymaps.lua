...
vim.keymap.set({ 'n', 'v' }, '<C-.>', vim.lsp.buf.code_action, { desc = 'LSP Code Action (Correção Rápida)' })

vim.keymap.set({ 'n', 'v' }, '<S-Tab>', '<<', { desc = 'Desindentar linha' })
vim.keymap.set({ 'n', 'v' }, '<tab>', '>>', { desc = 'Indentar linha' })
...
vim.keymap.set('n', '<C-y>', LazyVim.cmp.map({ 'ai_nes' }, '<tab>'), { desc = 'Sugestão de código (Sidekick)' })
